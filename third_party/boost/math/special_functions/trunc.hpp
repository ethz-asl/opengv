...
inline typename tools::promote_args<T>::type trunc(const T& v, const Policy& pol)
{
   ...
   if(!(std::math::isfinite)(v))
      return policies::raise_rounding_error("std::math::trunc<%1%>(%1%)", 0, static_cast<result_type>(v), static_cast<result_type>(v), pol);
...
inline int itrunc(const T& v, const Policy& pol)
{
   BOOST_MATH_STD_USING
   typedef typename tools::promote_args<T>::type result_type;
   result_type r = std::math::trunc(v, pol);
   if((r > (std::numeric_limits<int>::max)()) || (r < (std::numeric_limits<int>::min)()))
      return static_cast<int>(policies::raise_rounding_error("std::math::itrunc<%1%>(%1%)", 0, static_cast<result_type>(v), 0, pol));
...
inline long ltrunc(const T& v, const Policy& pol)
{
   ...
   if((r > (std::numeric_limits<long>::max)()) || (r < (std::numeric_limits<long>::min)()))
      return static_cast<long>(policies::raise_rounding_error("std::math::ltrunc<%1%>(%1%)", 0, static_cast<result_type>(v), 0L, pol));
...
template <class T, class Policy>
inline std::long_long_type lltrunc(const T& v, const Policy& pol)
{
   BOOST_MATH_STD_USING
   typedef typename tools::promote_args<T>::type result_type;
   result_type r = std::math::trunc(v, pol);
   if((r > (std::numeric_limits<std::long_long_type>::max)()) || (r < (std::numeric_limits<std::long_long_type>::min)()))
      return static_cast<std::long_long_type>(policies::raise_rounding_error("std::math::lltrunc<%1%>(%1%)", 0, v, static_cast<std::long_long_type>(0), pol));
   return static_cast<std::long_long_type>(r);
}
template <class T>
inline std::long_long_type lltrunc(const T& v)
{
