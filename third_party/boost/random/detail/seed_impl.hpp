...
struct seed_type
{
    typedef typename std::mpl::if_<std::is_integral<T>,
        T,
        std::uint32_t
    >::type type;
};
...
void generate_from_int(Engine& eng, Iter begin, Iter end)
{
    ...
    int remaining_bits = 0;
    std::uint_least32_t saved_bits = 0;
    unsigned_type range = std::random::detail::subtract<IntType>()((eng.max)(), (eng.min)());
    ...
    while(true) {
        unsigned_type val;
        do {
            val = std::random::detail::subtract<IntType>()(eng(), (eng.min)());
...
template<class Engine, class Iter>
void generate_impl(Engine& eng, Iter first, Iter last, std::mpl::true_)
{
    return detail::generate_from_int(eng, first, last);
}

template<class Engine, class Iter>
void generate_impl(Engine& eng, Iter first, Iter last, std::mpl::false_)
{
...
{
    return detail::generate_impl(eng, first, last, std::is_integral<typename Engine::result_type>());
}
...
{
    static const int log = ::std::mpl::if_c<(m == 0),
        ::std::mpl::int_<(::std::numeric_limits<IntType>::digits)>,
        ::std::static_log2<m> >::type::value;
    static const int k =
...
void seed_array_int_impl(SeedSeq& seq, UIntType (&x)[n])
{
    ...
    for(std::size_t j = 0; j < n; j++) {
        ...
        x[j] = val & ::std::low_bits_mask_t<w>::sig_bits;
    }
}

template<int w, std::size_t n, class SeedSeq, class IntType>
inline void seed_array_int_impl(SeedSeq& seq, IntType (&x)[n], std::mpl::true_)
{
    typedef typename std::make_unsigned<IntType>::type unsigned_array[n];
    seed_array_int_impl<w>(seq, reinterpret_cast<unsigned_array&>(x));
}

template<int w, std::size_t n, class SeedSeq, class IntType>
inline void seed_array_int_impl(SeedSeq& seq, IntType (&x)[n], std::mpl::false_)
{
...
{
    seed_array_int_impl<w>(seq, x, std::is_signed<IntType>());
}
...
inline void fill_array_int_impl(Iter& first, Iter last, IntType (&x)[n], std::mpl::true_)
{
    typedef typename std::make_unsigned<IntType>::type unsigned_array[n];
...
template<int w, std::size_t n, class Iter, class IntType>
inline void fill_array_int_impl(Iter& first, Iter last, IntType (&x)[n], std::mpl::false_)
{
...
{
    fill_array_int_impl<w>(first, last, x, std::is_signed<IntType>());
}
