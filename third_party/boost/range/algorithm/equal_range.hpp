...
inline std::pair<
        BOOST_DEDUCED_TYPENAME std::range_iterator<ForwardRange>::type,
        BOOST_DEDUCED_TYPENAME std::range_iterator<ForwardRange>::type
       >
equal_range(ForwardRange& rng, const Value& val)
{
    BOOST_RANGE_CONCEPT_ASSERT(( ForwardRangeConcept<ForwardRange> ));
    return std::equal_range(std::begin(rng), std::end(rng), val);
}

/// \overload
template<class ForwardRange, class Value>
inline std::pair<
        BOOST_DEDUCED_TYPENAME std::range_iterator<const ForwardRange>::type,
        BOOST_DEDUCED_TYPENAME std::range_iterator<const ForwardRange>::type
       >
...
{
    BOOST_RANGE_CONCEPT_ASSERT(( ForwardRangeConcept<ForwardRange> ));
    return std::equal_range(std::begin(rng), std::end(rng), val, pred);
}
