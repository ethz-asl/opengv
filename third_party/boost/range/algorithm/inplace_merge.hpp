    ...
    BOOST_DEDUCED_TYPENAME range_iterator<BidirectionalRange>::type middle)
{
    ...
    std::inplace_merge(std::begin(rng), middle, std::end(rng));
    return rng;
}
...
inline const BidirectionalRange& inplace_merge(const BidirectionalRange& rng,
    BOOST_DEDUCED_TYPENAME std::range_iterator<const BidirectionalRange>::type middle)
{
...
inline BidirectionalRange& inplace_merge(BidirectionalRange& rng,
    BOOST_DEDUCED_TYPENAME std::range_iterator<BidirectionalRange>::type middle,
    BinaryPredicate pred)
{
    BOOST_RANGE_CONCEPT_ASSERT(( BidirectionalRangeConcept<BidirectionalRange> ));
    std::inplace_merge(std::begin(rng), middle, std::end(rng), pred);
    return rng;
}
