...
inline OutputIterator copy_n(const SinglePassRange& rng, Size n, OutputIterator out)
{
    ...
    BOOST_ASSERT( n <= static_cast<Size>(::std::distance(rng)) );
    BOOST_ASSERT( n >= static_cast<Size>(0) );

    BOOST_DEDUCED_TYPENAME range_iterator<const SinglePassRange>::type source = ::std::begin(rng);
