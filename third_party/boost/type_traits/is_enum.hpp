...
# else
    template <class U> static ::std::type_traits::yes_type is_class_or_union_tester(void(U::*)(void));

#  if BOOST_WORKAROUND(BOOST_MSVC, == 1300)                 \
    || BOOST_WORKAROUND(__MWERKS__, <= 0x3000) // no SFINAE
    static ::std::type_traits::no_type is_class_or_union_tester(...);
    BOOST_STATIC_CONSTANT(
        bool, value = sizeof(is_class_or_union_tester(0)) == sizeof(::std::type_traits::yes_type));
#  else
    template <class U>
    static ::std::type_traits::no_type is_class_or_union_tester(...);
    BOOST_STATIC_CONSTANT(
        bool, value = sizeof(is_class_or_union_tester<T>(0)) == sizeof(::std::type_traits::yes_type));
#  endif
# endif
};
#endif
...
{
    template <typename T> struct type
       : public ::std::is_convertible<typename std::add_reference<T>::type,::std::detail::int_convertible>
    {
    };
};
...
{
   //typedef ::std::add_reference<T> ar_t;
   ...
   BOOST_STATIC_CONSTANT(bool, selector =
    ...
    typedef ::std::detail::is_enum_helper<
          ::std::detail::is_enum_impl<T>::selector
        > se_t;
#else
    typedef ::std::detail::is_enum_helper<selector> se_t;
#endif
...
BOOST_TT_AUX_BOOL_TRAIT_DEF1(is_enum,T,::std::detail::is_enum_impl<T>::value)
